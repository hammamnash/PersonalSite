...
The approved agentic AI exploration section sits after career history and before tools, keeping EA consulting as the professional headline. It separates current interests from job experience and uses this website as the concrete AI-assisted example, without invented metrics or expertise claims. Design Read remains ENERGY 2 / RHYTHM 2 / MOTION 1: existing DM Sans/Geist hierarchy and light neutrals, editorial copy beside a small conceptual workflow, then a plain example note. The workflow uses readable HTML rather than an image, with four steps in source order and arrows that communicate sequence. Human review is emphasized by the existing neutral panel tone; checkpoints before consequential actions are explicitly noted. No new brand hue, model logos, generic AI icons, or animation.

The Personal Projects section follows The Background (career history), before Agentic AI, and remains separate from employment. Five editorial rows preserve the owner's supplied names and visibly say “Details coming soon”; each links to a real static page under `app/projects/<slug>/page.tsx`. Rows stack the status beneath the title on narrow screens, using existing typography, neutral dividers, and 44px minimum targets. Placeholder pages reuse the floating nav, warm-to-cool hero, and one flat white notice panel. No guessed project details, metrics, screenshots, logos, live-app links, or launch dates. Independent page files support later content while a shared project layout and placeholder keep current styles consistent. See `docs/personal-projects.md`.

The homepage navigation follows section order: Work, About (The Background), Projects, Agentic AI, Tools, Contact. The monogram returns to the introduction. All six section links remain visible: one row on wide screens and two rows of three beside the monogram on narrow screens. Container queries let the navigation reflow at zoom; adequate scroll offset keeps target headings below the fixed header. Project-detail and 404 navigation retain their smaller recovery-focused sets. Native details/summary elements implement the numbered work accordions and work without JavaScript. No blog, CMS, database, login, or theme toggle. Email and LinkedIn are the only outbound contact actions.
...
Hover/focus feedback and user-triggered native smooth anchor scrolling only. Smooth navigation maintains orientation between sections and retains fixed-header clearance. Reduced-motion rules keep scrolling instant and disable transitions. Next.js is informed via `data-scroll-behavior="smooth"` so cross-page route restoration is not animated. No scroll-gated content or perpetual animation. 44px interactive targets, keyboard focus, and mobile/zoom reflow are release checks. A preview noindex rule is not an access-control mechanism.
