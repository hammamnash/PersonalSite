...
The Personal Projects section follows Selected Experience, before career history, and remains separate from employment. Five editorial rows preserve the owner's supplied names and visibly say “Details coming soon”; each links to a real static page under `app/projects/<slug>/page.tsx`. Rows stack the status beneath the title on narrow screens, using existing typography, neutral dividers, and 44px minimum targets. Placeholder pages reuse the floating nav, warm-to-cool hero, and one flat white notice panel. No guessed project details, metrics, screenshots, logos, live-app links, or launch dates. Independent page files support later content while a shared project layout and placeholder keep current styles consistent. See `docs/personal-projects.md`.
